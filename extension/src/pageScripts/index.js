...
const ajax_tools_space = {
  ...
  getOverrideText: (responseText, args, toJson = false) => {
    ...
    // console.log('getOverrideText', responseText, args);

    // 尝试将 args 发送给 background 处理
    try {
      // 由于 content script 无法直接访问 chrome.runtime.sendMessage，
      // 我们需要通过 window.postMessage 将消息发送到 content script，
      // 然后由 content script 转发给 background
      window.postMessage({
        type: 'ajaxTools',
        to: 'background',
        ...
        data: {
          ...
          args,
          toJson,
          extra: ajax_tools_space.extra,
        }
      }, '*');
    } catch (e) {
      console.error('发送数据到 background 失败:', e);
    }
